...
char get_comment_char_auto(const struct strbuf *sb)
{
	char candidates[] = "#;@!$%^&|:";
	char *candidate;
	const char *p;
...
const char *repo_get_comment_line_str(struct repository *repo, const struct strbuf *sb) {
    ...
    if (!repo_config_get_string_tmp(repo, "core.commentchar", &value) ||
        !repo_config_get_string_tmp(repo, "core.commentstring", &value)) {
        ...
        if (!strcasecmp(value, "auto")) {
            if (!sb)
                return "#";
            buf[0] = get_comment_char_auto(sb);
        } else {
            buf[0] = value[0];
        }
