				...
				struct repository *repo UNUSED)
{
	...
	if (!the_repository->sparse_checkout)
...
static int write_patterns_and_update(struct pattern_list *pl)
{
	...
	if (the_repository->sparse_checkout_cone)
...
static enum sparse_checkout_mode update_cone_mode(int *cone_mode) {
	...
	if (*cone_mode == -1 && the_repository->sparse_checkout)
		*cone_mode = the_repository->sparse_checkout_cone;
	...
	the_repository->sparse_checkout_cone = 0;
...
static int update_modes(int *cone_mode, int *sparse_index)
{
	...
	record_mode = (*cone_mode != -1) || !the_repository->sparse_checkout;
				   ...
				   int use_stdin)
{
	...
	repo_init_sparse_checkout(the_repository);
	existing.use_cone_patterns = the_repository->sparse_checkout_cone;
			   ...
			   const char *prefix, int skip_checks)
{
	...
	if (!args->nr)
		return;
	repo_init_sparse_checkout(the_repository);
	if (prefix && *prefix && the_repository->sparse_checkout_cone) {
		/*
			       ...
			       struct repository *repo UNUSED)
{
	 ...
	 */
	repo_init_sparse_checkout(the_repository);
	if (!the_repository->sparse_checkout_cone && !set_opts.use_stdin && argc == 0) {
				   ...
				   struct repository *repo UNUSED)
{
	...
	/*
	 * We do not exit early if !repo->sparse_checkout; due to the
