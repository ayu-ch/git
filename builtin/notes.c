...
static void write_commented_object(int fd, const struct object_id *object)
{
	...
	strbuf_add_commented_lines(&cbuf, buf.buf, buf.len, repo_get_comment_line_str(the_repository, NULL));
		...
		const struct object_id *old_note)
{
	if (d->use_editor || !d->msg_nr) {
		...
		if (d->stripspace)
			strbuf_stripspace(&d->buf, repo_get_comment_line_str(the_repository, NULL));
	}
}
